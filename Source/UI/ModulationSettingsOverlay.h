...
public:
        ...
        : modulationMatrix_(modMatrix), apvts_(apvts), fileLogger_(fileLogger), isVisible_(false)
    {
        ...
        // Licensing acknowledgements links
        acknowledgementsLabel_.setText("Thanks:", juce::dontSendNotification);
        acknowledgementsLabel_.setJustificationType(juce::Justification::centredLeft);
        acknowledgementsLabel_.setColour(juce::Label::textColourId, juce::Colour(0xFFB5B5B5));
        acknowledgementsLabel_.setFont(juce::Font(13.0f));
        addAndMakeVisible(acknowledgementsLabel_);

        braidsAcknowledgementsLink_.setTooltip("Open Mutable Instruments Braids source acknowledgements");
        braidsAcknowledgementsLink_.setColour(juce::HyperlinkButton::textColourId, juce::Colour(0xFF73C7A2));
        braidsAcknowledgementsLink_.setFont(juce::Font(13.0f), false);
        addAndMakeVisible(braidsAcknowledgementsLink_);

        acknowledgementsSeparatorLabel_.setText("|", juce::dontSendNotification);
        acknowledgementsSeparatorLabel_.setJustificationType(juce::Justification::centred);
        acknowledgementsSeparatorLabel_.setColour(juce::Label::textColourId, juce::Colour(0xFF9A9A9A));
        acknowledgementsSeparatorLabel_.setFont(juce::Font(13.0f));
        addAndMakeVisible(acknowledgementsSeparatorLabel_);

        juceAcknowledgementsLink_.setTooltip("Open JUCE GitHub repository");
        juceAcknowledgementsLink_.setColour(juce::HyperlinkButton::textColourId, juce::Colour(0xFF7DBBFF));
        juceAcknowledgementsLink_.setFont(juce::Font(13.0f), false);
        addAndMakeVisible(juceAcknowledgementsLink_);
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.drawLine(getWidth() / 2, 50, getWidth() / 2, getHeight() - 120, 1);
        g.drawLine(0, getHeight() - 120, getWidth(), getHeight() - 120, 1);
    }
    ...
    void resized() override
    {
        ...
        int globalY = bounds.getHeight() - 120;
        ...
        int panicY = bounds.getHeight() - 82;
        panicButton_.setBounds(bounds.getWidth() / 2 - 80, panicY, 160, 30);

        // Acknowledgements links at very bottom
        int ackY = bounds.getHeight() - 42;
        acknowledgementsLabel_.setBounds(10, ackY, 60, 20);
        braidsAcknowledgementsLink_.setBounds(72, ackY, 170, 20);
        acknowledgementsSeparatorLabel_.setBounds(246, ackY, 12, 20);
        juceAcknowledgementsLink_.setBounds(262, ackY, 50, 20);
    }
...
private:
    ...
    juce::Label acknowledgementsLabel_;
    juce::Label acknowledgementsSeparatorLabel_;
    juce::HyperlinkButton braidsAcknowledgementsLink_ {
        "Mutable Instruments",
        juce::URL("https://github.com/pichenettes/eurorack/tree/master/braids")
    };
    juce::HyperlinkButton juceAcknowledgementsLink_ {
        "JUCE",
        juce::URL("https://github.com/juce-framework/JUCE")
    };
